<x-admin-layout>
    ...
    <div id="modalTambah" class="fixed inset-0 bg-gray-900/60 backdrop-blur-md hidden flex items-center justify-center z-[100] transition-all">
        <div class="bg-white p-10 rounded-[3rem] shadow-2xl w-full max-w-lg border border-gray-100 relative overflow-hidden">
            <div class="absolute top-0 right-0 -mr-16 -mt-16 w-32 h-32 bg-emerald-500 rounded-full blur-[60px] opacity-20"></div>
            ...
            <form action="{{ route('admin.beasiswa.store') }}" method="POST" class="space-y-6 relative z-10">
                ...
                <div>
                    ...
                    <input type="text" name="nama" required class="w-full border-none bg-gray-50 rounded-2xl focus:ring-2 focus:ring-emerald-500 font-bold text-gray-700 p-4 transition-all">
                </div>
                
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label class="block text-[10px] font-black uppercase text-gray-400 mb-2 tracking-widest">Kuota</label>
                        <input type="number" name="kuota" required class="w-full border-none bg-gray-50 rounded-2xl focus:ring-2 focus:ring-emerald-500 font-bold text-gray-700 p-4 transition-all">
                    </div>
                    <div>
                        <label class="block text-[10px] font-black uppercase text-gray-400 mb-2 tracking-widest">Periode</label>
                        <input type="text" name="periode" placeholder="2025/2026" required class="w-full border-none bg-gray-50 rounded-2xl focus:ring-2 focus:ring-emerald-500 font-bold text-gray-700 p-4 transition-all">
                    </div>
                </div>

                <div>
                    <label class="block text-[10px] font-black uppercase text-gray-400 mb-2 tracking-widest">Deskripsi Singkat</label>
                    <textarea name="deskripsi" class="w-full border-none bg-gray-50 rounded-2xl focus:ring-2 focus:ring-emerald-500 font-bold text-gray-700 p-4 transition-all" rows="3"></textarea>
                </div>

                <div class="flex items-center gap-4 mt-8">
                    <button type="button" onclick="document.getElementById('modalTambah').classList.add('hidden')" 
                            class="flex-1 py-4 text-gray-400 font-black uppercase text-[10px] tracking-widest hover:text-rose-500 transition-colors">
                        Batal
                    </button>
                    <button type="submit" 
                            class="flex-[2] py-4 bg-gray-900 text-white rounded-2xl font-black shadow-xl shadow-gray-200 uppercase text-[10px] tracking-widest hover:bg-emerald-600 hover:-translate-y-1 transition-all">
                        Simpan Program
                    </button>
                </div>
            </form>
